...
static void L3_decode_scalefactors(mp3dec_work_t *work, const uint8_t *hdr, uint8_t *ist_pos, bs_t *bs, const L3_gr_info_t *gr, float *scf, int ch)
{
    ...
    /* uint8_t scf_size[4], iscf[40]; */ /* TODO */
    ...
    if (HDR_TEST_MPEG1(hdr))
    {
        ...
        work->scf_size[1] = work->scf_size[0] = (uint8_t)(part >> 2);
        work->scf_size[3] = work->scf_size[2] = (uint8_t)(part & 3);
    } else
    {
        ...
        for (k = ist*3*4; sfc >= 0; sfc -= modprod, k += 4)
        {
            ...
            {
                work->scf_size[i] = (uint8_t)(sfc / modprod % g_mod[k + i]);
                modprod *= g_mod[k + i];
            }
        }
        scf_partition += k;
    ...
    L3_read_scalefactors(work->iscf, ist_pos, work->scf_size, scf_partition, bs, scfsi);
    ...
    if (gr->n_short_sfb)
    {
        ...
        {
            work->iscf[gr->n_long_sfb + i + 0] += gr->subblock_gain[0] << sh;
            work->iscf[gr->n_long_sfb + i + 1] += gr->subblock_gain[1] << sh;
            work->iscf[gr->n_long_sfb + i + 2] += gr->subblock_gain[2] << sh;
        }
    } else if (gr->preflag)
    {
    ...
    {
        scf[i] = L3_ldexp_q2(gain, work->iscf[i] << scf_shift);
    }
}
...
static void L3_imdct36(mp3dec_work_t *work, float *grbuf, float *overlap, const float *window, int nbands)
{
    int i, j;
    static const float g_twid9[18] = {
        0.73727734f,0.79335334f,0.84339145f,0.88701083f,0.92387953f,0.95371695f,0.97629601f,0.99144486f,0.99904822f,0.67559021f,0.60876143f,0.53729961f,0.46174861f,0.38268343f,0.30070580f,0.21643961f,0.13052619f,0.04361938f
    };
    float *co = work->co, *si = work->si;

    for (j = 0; j < nbands; j++, grbuf += 18, overlap += 9)
    {
        /* float co[9], si[9]; */ /* TODO */
        co[0] = -grbuf[0];
        si[0] = grbuf[17];
        for (i = 0; i < 4; i++)
        {
...
static void L3_imdct12(mp3dec_work_t *work, float *x, float *dst, float *overlap)
{
    static const float g_twid3[6] = { 0.79335334f,0.92387953f,0.99144486f, 0.60876143f,0.38268343f,0.13052619f };
    /*float co[3], si[3];*/ /* TODO */
    float *co = work->co, *si = work->si;
    int i;
...
static void L3_imdct_short(mp3dec_work_t *work, float *grbuf, float *overlap, int nbands)
{
    float *tmp = work->tmp;
    for (;nbands > 0; nbands--, overlap += 9, grbuf += 18)
    {
        /* float tmp[18]; */ /* TODO */
        memcpy(tmp, grbuf, sizeof(float[18]));
        memcpy(grbuf, overlap, 6*sizeof(float));
        L3_imdct12(work, tmp, grbuf + 6, overlap + 6);
        L3_imdct12(work, tmp + 1, grbuf + 12, overlap + 6);
        L3_imdct12(work, tmp + 2, overlap, overlap + 6);
    }
}
...
static void L3_imdct_gr(mp3dec_work_t *work, float *grbuf, float *overlap, unsigned block_type, unsigned n_long_bands)
{
    ...
    if (n_long_bands)
    {
        L3_imdct36(work, grbuf, overlap, g_mdct_window[0], n_long_bands);
        grbuf += 18*n_long_bands;
        overlap += 9*n_long_bands;
    }
    if (block_type == SHORT_BLOCK_TYPE)
        L3_imdct_short(work, grbuf, overlap, 32 - n_long_bands);
    else
        L3_imdct36(work, grbuf, overlap, g_mdct_window[block_type == STOP_BLOCK_TYPE], 32 - n_long_bands);
}
...
static void L3_decode(mp3dec_t *h, mp3dec_work_t *work, mp3dec_scratch_t *s, L3_gr_info_t *gr_info, int nch)
{
    int ch;
    ...
    for (ch = 0; ch < nch; ch++)
    {
        ...
        L3_decode_scalefactors(work, h->header, s->ist_pos[ch], &s->bs, gr_info + ch, s->scf, ch);
    ...
    for (ch = 0; ch < nch; ch++, gr_info++)
    {
        ...
        L3_imdct_gr(work, s->grbuf[ch], h->mdct_overlap[ch], gr_info->block_type, n_long_bands);
        L3_change_sign(s->grbuf[ch]);
    }
}

static void mp3d_DCT_II(mp3dec_work_t *work, float *grbuf, int n)
{
    ...
    for (; k < n; k++)
    {
        /*float t[4][8], *x, *y = grbuf + k;*/  /* TODO */
        float *x, *y = grbuf + k;

        for (x = work->t[0], i = 0; i < 8; i++, x++)
        {
            float x0 = y[i*18];
            float x1 = y[(15 - i)*18];
            float x2 = y[(16 + i)*18];
        ...
        {
            y[0*18] = work->t[0][i];
            y[1*18] = work->t[2][i] + work->t[3][i] + work->t[3][i + 1];
            y[2*18] = work->t[1][i] + work->t[1][i + 1];
            y[3*18] = work->t[2][i + 1] + work->t[3][i] + work->t[3][i + 1];
        }
        y[0*18] = work->t[0][7];
        y[1*18] = work->t[2][7] + work->t[3][7];
        y[2*18] = work->t[1][7];
        y[3*18] = work->t[3][7];
    }
}

static int16_t mp3d_scale_pcm(float gain, float sample)
{
    sample = sample * gain;
...
static void mp3d_synth_pair(float gain, mp3d_sample_t *pcm, int nch, const float *z)
{
    float a;
    a  = (z[14*64] - z[    0]) * 29;
    ...
    a += z[ 2*64] * 146;
    a += z[ 0*64] * -5;
    pcm[16*nch] = mp3d_scale_pcm(gain, a);
}

static void mp3d_synth(float gain, mp3dec_work_t *work, float *xl, mp3d_sample_t *dstl, int nch, float *lins)
{
    int i;
    ...
    mp3d_synth_pair(gain, dstr, nch, lins + 4*15 + 1);
    mp3d_synth_pair(gain, dstr + 32*nch, nch, lins + 4*15 + 64 + 1);
    mp3d_synth_pair(gain, dstl, nch, lins + 4*15);
    mp3d_synth_pair(gain, dstl + 32*nch, nch, lins + 4*15 + 64);
...
#endif
#define LOAD(k) work->w0 = *w++; work->w1 = *w++; vz = &zlin[4*i - k*64]; vy = &zlin[4*i - (15 - k)*64];
#define S0(k) { int j; LOAD(k); for (j = 0; j < 4; j++) work->b[j]  = vz[j]*work->w1 + vy[j]*work->w0, work->a[j]  = vz[j]*work->w0 - vy[j]*work->w1; }
#define S1(k) { int j; LOAD(k); for (j = 0; j < 4; j++) work->b[j] += vz[j]*work->w1 + vy[j]*work->w0, work->a[j] += vz[j]*work->w0 - vy[j]*work->w1; }
#define S2(k) { int j; LOAD(k); for (j = 0; j < 4; j++) work->b[j] += vz[j]*work->w1 + vy[j]*work->w0, work->a[j] += vy[j]*work->w1 - vz[j]*work->w0; }
        /* float a[4], b[4]; */  /* TODO */
        /* float w0; float w1; */
        float *vz; float *vy;
        ...
        dstr[(15 - i)*nch] = mp3d_scale_pcm(gain, work->a[1]);
        dstr[(17 + i)*nch] = mp3d_scale_pcm(gain, work->b[1]);
        dstl[(15 - i)*nch] = mp3d_scale_pcm(gain, work->a[0]);
        dstl[(17 + i)*nch] = mp3d_scale_pcm(gain, work->b[0]);
        dstr[(47 - i)*nch] = mp3d_scale_pcm(gain, work->a[3]);
        dstr[(49 + i)*nch] = mp3d_scale_pcm(gain, work->b[3]);
        dstl[(47 - i)*nch] = mp3d_scale_pcm(gain, work->a[2]);
        dstl[(49 + i)*nch] = mp3d_scale_pcm(gain, work->b[2]);
    }
}

static void mp3d_synth_granule(float gain, mp3dec_work_t *work, float *qmf_state, float *grbuf, int nbands, int nch, mp3d_sample_t *pcm, float *lins)
{
    int i;
    for (i = 0; i < nch; i++)
    {
        mp3d_DCT_II(work, grbuf + 576*i, nbands);
    }
    ...
    {
        mp3d_synth(gain, work, grbuf + i, pcm + 32*nch*i, nch, lins + i*64);
    }
    if (nch == 1)
    {
        for (i = 0; i < 15*64; i += 2)
...
int mp3dec_decode_frame(float gain, mp3dec_t *dec, mp3dec_work_t *work, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    bs_init(&work->bs_frame, hdr + HDR_SIZE, frame_size - HDR_SIZE);
    if (HDR_IS_CRC(hdr))
    {
        get_bits(&work->bs_frame, 16);
    }

    if (info->layer == 3)
    {
        int main_data_begin = L3_read_side_info(&work->bs_frame, work->scratch.gr_info, hdr);
        if (main_data_begin < 0 || work->bs_frame.pos > work->bs_frame.limit)
        {
            mp3dec_init(dec);
            return 0;
        }
        success = L3_restore_reservoir(dec, &work->bs_frame, &work->scratch, main_data_begin);
        if (success)
        {
            for (igr = 0; igr < (HDR_TEST_MPEG1(hdr) ? 2 : 1); igr++, pcm += 576*info->channels)
            {
                memset(work->scratch.grbuf[0], 0, 576*2*sizeof(float));
                L3_decode(dec, work, &work->scratch, work->scratch.gr_info + igr*info->channels, info->channels);
                mp3d_synth_granule(gain, work, dec->qmf_state, work->scratch.grbuf[0], 18, info->channels, pcm, work->scratch.syn[0]);
            }
        }
        L3_save_reservoir(dec, &work->scratch);
    } else
    {
        /* L12_scale_info sci[1]; */
        L12_read_scale_info(hdr, &work->bs_frame, &work->sci);

        memset(work->scratch.grbuf[0], 0, 576*2*sizeof(float));
        for (i = 0, igr = 0; igr < 3; igr++)
        {
            if (12 == (i += L12_dequantize_granule(work->scratch.grbuf[0] + i, &work->bs_frame, &work->sci, info->layer | 1)))
            {
                i = 0;
                L12_apply_scf_384(&work->sci, work->sci.scf + igr, work->scratch.grbuf[0]);
                mp3d_synth_granule(gain, work, dec->qmf_state, work->scratch.grbuf[0], 12, info->channels, pcm, work->scratch.syn[0]);
                memset(work->scratch.grbuf[0], 0, 576*2*sizeof(float));
                pcm += 384*info->channels;
            }
            if (&work->bs_frame.pos > &work->bs_frame.limit)
            {
                mp3dec_init(dec);
                return 0;
            }
